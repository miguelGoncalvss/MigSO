...
void I_InitGraphics (void)
{
    ...
    static byte static_video_buffer[SCREENWIDTH * SCREENHEIGHT];
	I_VideoBuffer = static_video_buffer;
...
extern void DG_SetPalette(const uint8_t* palette);

void I_SetPalette (byte* palette)
{
	int i;
	uint8_t dac_pal[256 * 3];
    ...
    for (i=0; i<256; ++i ) {
        ...
        dac_pal[i * 3 + 0] = colors[i].r;
        dac_pal[i * 3 + 1] = colors[i].g;
        dac_pal[i * 3 + 2] = colors[i].b;
    }
